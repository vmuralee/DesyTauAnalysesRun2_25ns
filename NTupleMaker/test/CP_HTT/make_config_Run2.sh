...
if [[ $YEAR -eq 16 && $DATA_TYPE == "MC" ]]; then
  ...
  # NB: we use non-CP5 SF file but correct SFs with old/new eff ratio
  # tagging_efficiencies_deepCSV_2016_CP5.root is taken from Danny: /afs/cern.ch/work/d/dwinterb/public/btag_efficiencies_cp5/
  ...
  # setting ApplyBTagCP5Correction to true for SAMPLE_POSTFIX config
  sed -i 's/^ApplyBTagCP5Correction.*$/ApplyBTagCP5Correction = true/g' $OUTDIR/${TEMPLATE_CFG_NAME}_${SAMPLE_POSTFIX}.conf
  
fi # END if [[ $YEAR -eq 16 && $DATA_TYPE == "MC" ]]; then
